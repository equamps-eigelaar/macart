...
import { Plus, Search, Download, ShieldCheck } from "lucide-react";
import { format } from "date-fns";

const STATUS_COLORS = {
  open: "bg-red-500/15 text-red-400",
  quarantine: "bg-orange-500/15 text-orange-400",
  disposition: "bg-amber-500/15 text-amber-400",
  root_cause: "bg-blue-500/15 text-blue-400",
  capa_open: "bg-purple-500/15 text-purple-400",
  verify: "bg-cyan-500/15 text-cyan-400",
  closed: "bg-green-500/15 text-green-400",
};

const SOURCES = ["incoming_rm","in_process","final_inspection","customer_complaint","audit","other"];

const emptyForm = { ncr_number: "", title: "", source: "in_process", description: "", detected_at: "", detected_date: format(new Date(), "yyyy-MM-dd"), status: "open", disposition: "", detected_by: "", notes: "" };
...
export default function NCRRegister() {
  ...
  const [creatingCapa, setCreatingCapa] = useState(null);

  const load = async () => { const d = await base44.entities.NCR.list("-detected_date", 200); setNcrs(d); };
  useEffect(() => { load(); }, []);
  ...
  const createCapa = async (ncr) => {
    setCreatingCapa(ncr.id);
    try {
      const dueDate = new Date(); dueDate.setDate(dueDate.getDate() + 30);
      await base44.entities.CAPA.create({
        capa_number: `CAPA-${Date.now().toString().slice(-6)}`,
        ncr_id: ncr.id,
        title: `CAPA for ${ncr.ncr_number}: ${ncr.title || ncr.description?.slice(0, 60)}`,
        type: "corrective",
      ...
      await base44.entities.NCR.update(ncr.id, { status: "capa_open" });
      alert("CAPA created. Go to CAPA Tracker to complete it.");
      load();
    } finally {
  ...
  const filtered = ncrs.filter(n => {
    const ms = !search || n.ncr_number?.toLowerCase().includes(search.toLowerCase()) || n.title?.toLowerCase().includes(search.toLowerCase()) || n.description?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">NCR Register</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {ncrs.length} total
            {openCount > 0 && <span className="text-red-400 ml-2">· {openCount} open</span>}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => exportCSV(filtered, "ncr-register.csv")}
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Date Detected</label>
              <input type="date" value={form.detected_date || ""} onChange={e => set("detected_date", e.target.value)}
                className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.status} onChange={e => set("status", e.target.value)}
                className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary">
                {["open","quarantine","disposition","root_cause","capa_open","verify","closed"].map(s => <option key={s} value={s}>{s.replace(/_/g," ")}</option>)}
              </select>
            </div>
            <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 && <tr><td colSpan={6} className="text-center py-12 text-muted-foreground">No NCRs found</td></tr>}
              {filtered.map(n => (
                <tr key={n.id} className="hover:bg-secondary/40">
                  <td className="px-4 py-3 font-mono font-medium">{n.ncr_number}</td>
                  <td className="px-4 py-3 max-w-xs truncate">{n.title || n.description?.slice(0, 50)}</td>
                  <td className="px-4 py-3 capitalize text-muted-foreground">{n.source?.replace(/_/g," ")}</td>
                  <td className="px-4 py-3 text-muted-foreground">{n.detected_date || n.created_date?.slice(0,10)}</td>
                  <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-md text-xs font-medium ${STATUS_COLORS[n.status]}`}>{n.status?.replace(/_/g," ")}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <button onClick={() => openEdit(n)} className="text-xs text-primary hover:underline">Edit</button>
                      {(n.status === "open" || n.status === "root_cause" || n.status === "disposition") && (
                        <button
                          onClick={() => createCapa(n)}
                          disabled={creatingCapa === n.id}
                          className="flex items-center gap-1 text-xs text-purple-400 hover:underline disabled:opacity-50"
                        >
                          <ShieldCheck className="w-3 h-3" />
                          {creatingCapa === n.id ? "Creating…" : "Create CAPA"}
                        </button>
                      )}
                    </div>
                  </td>
