...
const empty = { customer_code:"", name:"", contact_name:"", email:"", phone:"", address:"", status:"active", credit_limit:"", payment_terms:"", notes:"" };
...
export default function Customers() {
  ...
  const load = async () => { const d = await base44.entities.Customer.list(); setRecords(d); };
  useEffect(() => { load(); }, []);
  ...
  const filtered = records.filter(r => !search || r.name?.toLowerCase().includes(search.toLowerCase()) || r.customer_code?.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Customers</h1><p className="text-sm text-muted-foreground mt-0.5">{records.length} customers</p></div>
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[{l:"Code *",k:"customer_code"},{l:"Name *",k:"name"},{l:"Contact Name",k:"contact_name"},{l:"Email",k:"email"},{l:"Phone",k:"phone"},{l:"Payment Terms",k:"payment_terms"}].map(f => (
            ...
            <div className="sm:col-span-2"><label className="text-xs text-muted-foreground mb-1 block">Address</label>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 && <tr><td colSpan={7} className="text-center py-12 text-muted-foreground">No customers found</td></tr>}
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-secondary/40">
                  <td className="px-4 py-3 font-mono font-bold">{r.customer_code}</td>
